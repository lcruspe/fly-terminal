...
После 30 секунд без мыши, клавиатуры, скролла или clipboard сервер временно снижает поток до минимальных 15 FPS. Первое реальное пользовательское действие возвращает выбранный FPS cap. Технические `configure`, probe и WebRTC bridge keepalive не считаются пользовательской активностью. Таймаут отключения задаётся в HUD (1–60 минут), сохраняется локально и применяется и на сервере, и на клиенте; клиентский контроль нужен, чтобы WebRTC bridge не мог прозрачно восстановить уже завершённый по idle сеанс.
