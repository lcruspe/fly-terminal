...
| `FLY_DESKTOP_IDLE_FPS_AFTER_SECONDS` | `30` | Через сколько секунд без пользовательского ввода включать глубокий idle-профиль 640×360 @ 5 FPS, 300 кбит/с. Первое действие возвращает сохранённые рабочие параметры. |
...
Для Remote Desktop без параметра `display` захватывается именно **главный дисплей macOS** (`CGMainDisplayID()`), а не первый дисплей из списка ScreenCaptureKit. Это важно на экране блокировки: поле ввода пароля находится на главном дисплее и остаётся доступным удалённо. Явные `display=Fly Remote` и `display=Fly Browser` по-прежнему выбирают соответствующие виртуальные дисплеи. Тип H.264-клиента задаётся в URL явно параметром `client=desktop|browser`; имя захватываемого дисплея больше не определяет fallback-маршрут. При заблокированной macOS ScreenCaptureKit может выдавать валидный H.264-поток с пустыми/чёрными кадрами; клиент определяет screenLocked из init/host_locked и немедленно переводит Remote Desktop на RDC VNC, который используется для экрана входа вместо показа чёрного Native-потока.
...
Через 30 секунд без мыши, клавиатуры, скролла или clipboard H.264 streamer автоматически включает глубокий idle-профиль 640×360 @ 5 FPS с bitrate 300 кбит/с, не меняя сохранённые рабочие параметры.

Native RDC синхронизирует текстовый буфер обмена в обе стороны штатными сочетаниями клиента: на Windows/Linux Ctrl+C/Ctrl+V, на macOS Cmd+C/Cmd+V. При копировании клиент отправляет отдельную clipboard_pull-команду, сервер выполняет Cmd+C на Mac и возвращает текст pasteboard в системный clipboard браузера. При вставке текст сначала переносится в macOS pasteboard, затем сервер выполняет Cmd+V, поэтому Unicode не зависит от раскладки клавиатуры. Файлы, которые браузер предоставляет в ClipboardEvent.clipboardData.files, передаются клиент→Mac чанками по надёжному каналу, записываются во временный каталог и помещаются в NSPasteboard как реальные file URL перед Cmd+V; действуют лимиты FLY_CLIPBOARD_MAX_FILES, FLY_CLIPBOARD_MAX_FILE_BYTES и FLY_CLIPBOARD_MAX_TOTAL_BYTES. Обратное помещение произвольных файлов Mac→Windows в системный файловый clipboard веб-клиент не эмулирует: браузерный Clipboard API не предоставляет Windows CF_HDROP; для такого направления нужен отдельный нативный клиентский clipboard bridge.
 Первое пользовательское действие возвращает выбранные разрешение, FPS cap и bitrate. Ограничение FPS и разрешение потока можно менять прямо в HUD; те же настройки из общего Settings применяются к уже открытым H.264-вкладкам через `postMessage`/WebSocket без перезагрузки iframe. Смена разрешения использует штатный `configure_encoder`: encoder перезапускается внутри существующей транспортной сессии, поэтому вкладка и управление не разрываются.
