...
Текущая реализация рассчитана на прокрутку `terminal scrollback`, а не всей страницы браузера. Если поведение выглядит старым, сделайте hard reload, потому что shell UI и ttyd theme/script кешируются браузером отдельно.
