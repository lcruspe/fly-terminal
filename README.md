...
HUD H.264-клиента по умолчанию свёрнут до небольшого статусного chip. Полная панель открывается только с самого chip, автоматически скрывается через 3 секунды без взаимодействия, может быть закреплена и перетащена; закрепление и позиция сохраняются локально в браузере. Наведение на удалённый экран само по себе HUD больше не показывает.
